...
def _seed_basic(conn, embedder, project, corpus_dir):
    scan_path(conn, project.id, corpus_dir, embedder=embedder)


def test_lex_retrieval_returns_matching_doc(conn, fake_embedder, project, corpus_dir):
    _seed_basic(conn, fake_embedder, project, corpus_dir)
...
def test_retrieval_excludes_other_projects(conn, fake_embedder, corpus_dir):
    ...
    scan_path(conn, p1.id, corpus_dir, embedder=fake_embedder)
    # p2 has nothing — retrieve should return empty
    r = Retriever(conn, embedder=fake_embedder).retrieve(
...
def test_anchored_ppr_boosts_anchor_neighbours(conn, fake_embedder, project, corpus_dir):
    _seed_basic(conn, fake_embedder, project, corpus_dir)
...
def test_status_filter_excludes_dismissed(conn, fake_embedder, project, corpus_dir):
    _seed_basic(conn, fake_embedder, project, corpus_dir)
...
def test_why_string_mentions_lex_terms(conn, fake_embedder, project, corpus_dir):
    _seed_basic(conn, fake_embedder, project, corpus_dir)
    r = Retriever(conn, embedder=fake_embedder).retrieve(
